...
const navigate = useNavigate();
  ...
  const handleSubmit = async (e) => {
    ...
    if (data.password === data.confirmPassword) {
      ...
      console.log("Check password and confirm it !");
    }
  }
  return (
    <section id='signup' className="flex items-center container">
      <div className="w-full block bg-white shadow-lg p-8 m-4 md:max-w-sm md:mx-auto">
        <div className="w-28 h-28 text-xl text-center uppercase mb-4 relative overflow-hidden rounded-full mx-auto">
          <div className="">
            <img src={data.profileAvatar || loginIcon} alt="" className='bg-white mx-auto' />
          </div>
          <form action="">
            <label>
              ...
              <input className='hidden absolute bottom-2 left-0'
                ref={fileInputRef}
        ...
        <form className="mb-4 mt-6 flex-col" method="post" onSubmit={handleSubmit}>
          ...
          <div className="text-center mt-5">
            <button disabled={!isFormValid} className="bg-blue-500 w-28 hover:bg-green-700 text-white uppercase max-w-[150px] hover:scale-110 transition-all text-sm font-semibold px-4 py-2 rounded-full">Sign up</button>
          </div>
        </form>
        <div className="text-center">

          <p className='my-3'> Already have account ? <Link to={"/login"} className="text-red-500 hover:text-red-700">Login</Link></p>
        </div>
      </div>
    </section>
  )
